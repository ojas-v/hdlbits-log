...
module top_module (
    ...
    // The heater is driven HIGH only in heating mode (mode=1) AND when too cold.
    assign heater = mode & too_cold;

    // The aircon is driven HIGH only in cooling mode (mode=0) AND when too hot.
    assign aircon = ~mode & too_hot;

    // The fan is driven HIGH if the heater is on, OR the aircon is on, OR the manual override is triggered.
